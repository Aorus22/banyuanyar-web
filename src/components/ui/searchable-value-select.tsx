...
export function SearchableValueSelect({
  ...
  const filteredItems = React.useMemo(() => {
    ...
    return items.filter(item =>
      item.label.toLowerCase().includes(s) || item.value.toLowerCase().includes(s)
    )
  }, [items, search])
  ...
  const totalHeight = React.useMemo(() => uniqueItems.length * ITEM_HEIGHT, [uniqueItems.length])
  const startIndex = React.useMemo(() => Math.max(0, Math.floor(scrollTop / ITEM_HEIGHT) - BUFFER_ITEMS), [scrollTop])
  const endIndex = React.useMemo(() => Math.min(
    uniqueItems.length,
    startIndex + VISIBLE_ITEMS + (BUFFER_ITEMS * 2)
  ), [uniqueItems.length, startIndex])
  const visibleItems = React.useMemo(() => uniqueItems.slice(startIndex, endIndex), [uniqueItems, startIndex, endIndex])
  ...
  const selectItem = React.useCallback((item: Item) => {
    if (!disabled && !loading) {
    ...
    if (!disabled && !loading) {
      ...
      if (newOpen) {
        ...
        setTimeout(() => {
          ...
          const selectedIndex = uniqueItems.findIndex(item => item.value === value)
          if (dropdownRef.current) {
            const el = dropdownRef.current.querySelector(".virtual-scroll") as HTMLElement
            if (el) {
              if (selectedIndex !== -1) {
  ...
  }, [disabled, loading, isOpen, resetSearch, updateListWidth, checkDropdownPosition, uniqueItems, value])
  ...
  return (
    <div className={cn("relative w-full", className)}>
      <button
        ref={anchorRef}
        className={cn(
          "bg-card text-foreground shadow-xs border border-input justify-between w-full flex items-center px-3 py-1 min-h-9 h-9 text-sm rounded-lg",
          triggerStyle,
          {
        ...
        type="button"
      >
        <div className="flex items-center gap-2 truncate overflow-hidden whitespace-nowrap flex-1">
          {loading && <Loader2 className="h-4 w-4 animate-spin" />}
          <span className={cn({ "text-muted-foreground": isPlaceholder || disabled })}>
            {selectedLabel}
          </span>
        </div>
      ...
      {isOpen && (
        <div
          ref={dropdownRef}
          className={cn(
            "absolute z-50 mt-1 rounded-lg border bg-popover text-popover-foreground shadow-md outline-none",
        ...
        >
          <div className="relative w-full items-center">
            <input
              className="focus-visible:ring-0 rounded-md h-10 w-full border-b pl-9 pr-3 py-1 outline-none text-sm"
              placeholder="Cari..."
              disabled={loading || disabled}
              value={search}
            ...
            />
            <span className="absolute left-0 inset-y-0 flex items-center justify-center pl-3 pointer-events-none">
                ...
                <div style={{ transform: `translateY(${offsetY}px)` }}>
                  {visibleItems.map((item) => (
                    <div
                      key={item.uniqueKey}
                      className={cn(
                        "min-h-9 text-sm px-3 py-1.5 border-b border-border last:border-b-0 flex items-center cursor-pointer",
                        {
                    ...
                    >
                      <span className={cn("flex-1", { "text-muted-foreground": disabled })}>
                        {item.label}
                      </span>
                      {item.value === value && <Check className="ml-auto h-4 w-4" />}
                    </div>
                  ))}
                </div>
              </div>
