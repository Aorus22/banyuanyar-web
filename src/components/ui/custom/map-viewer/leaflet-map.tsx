...
export default function LeafletMap({ umkms, height = "400px", className = "" }: LeafletMapProps) {
  ...
  useEffect(() => {
    ...
    if (umkms.length > 1) {
      const group = new L.FeatureGroup(umkms.map(u => L.marker([u.latitude, u.longitude])))
