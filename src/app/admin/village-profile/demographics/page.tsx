...
export default async function DemographicsPage() {
  const profiles = await getVillageProfileByKeys(['demographics_data']);

  const demographicsData =
    profiles.find((p) => p.key === 'demographics_data')?.value || '';
  ...
  return (
    <div className='space-y-6'>
      ...
      <DemographicsForm demographicsData={demographicsData} />
    </div>
  );
}
