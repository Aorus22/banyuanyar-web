...
export function DemographicsForm({ 
  ...
  const [demographicsContent, setDemographicsContent] = useState<Content>(demographicsData);
  const [isLoading, setIsLoading] = useState(false);

  const handleSave = async () => {
    const demographicsText = typeof demographicsContent === 'string' ? demographicsContent : JSON.stringify(demographicsContent);
    ...
    try {
      const updates = [
        { key: 'demographics_data', value: demographicsText }
      ];

      const result = await updateMultipleVillageProfiles(updates);
      
      if (result.success) {
        toast.success("Informasi demografi & geografis desa berhasil diperbarui");
      } else {
        toast.error(result.error || "Gagal memperbarui informasi demografi & geografis desa");
      }
    } catch (error) {
