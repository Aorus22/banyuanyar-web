...
const formatFileSize = (bytes: bigint | null) => {
  ...
  return (
    Math.round((Number(bytes) / Math.pow(1024, i)) * 100) / 100 + ' ' + sizes[i]
  );
};
...
export const columns: ColumnDef<Document>[] = [
  ...
  },
  {
    ...
    header: 'Nama File',
    cell: ({ row }) => (
      <div className='flex max-w-[300px] min-w-[200px] items-center gap-2'>
        <FileText className='h-4 w-4 flex-shrink-0 text-blue-500' />
        <span className='truncate text-sm break-all'>
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (result.success) {
                      ...
                      toast.error(
                        `Gagal menghapus dokumen: ${result.error || 'Unknown error'}`,
                        { id: loadingToast }
                      );
                    }
                  } catch (error) {
