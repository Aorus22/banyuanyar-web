...
import { createGovernmentOfficial } from '../server-action';
...
export function GovernmentOfficialForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await createGovernmentOfficial(formData);

      if (result.success) {
        toast.success('Perangkat desa berhasil ditambahkan');
        router.push('/admin/government-officials');
      } else {
        toast.error(result.error || 'Gagal menambahkan perangkat desa');
      }
    } catch (error) {
